...
function Groundskeeper(options) {
    ...
    this.options.namespace = [].concat(this.options.namespace);
    this.options.replace = this.options.replace || '';
}
...
Groundskeeper.prototype.write = function (data) {
    ...
    if (data && data.length) {
        buffer += this.clean(data.toString());
...
Groundskeeper.prototype.removeNamespace = function (node, namespace) {
    ...
    if ([type.MemberExpression, type.Identifier].indexOf(node.type) !== -1 &&
        ...
        while ([type.CallExpression, type.AssignmentExpression].indexOf(node.type) === -1) {
            node = node.parent;
        }
